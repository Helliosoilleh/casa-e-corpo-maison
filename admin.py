...
# ========= FUNÇÃO QUE SALVA NA LOJA DE VERDADE (NOVA) =========
...
# ========= SEU CÓDIGO ORIGINAL (MANTIVE TUDO) =========
def get_senha():
    try:
...
[data-testid="stSidebar"]{background:linear-gradient(#121212, #2D2D2D)!important;border-right:1px solid #D4AF37}
</style>
...
# ========= MARKETING (AGORA CONSERTADO) =========
if menu == "MARKETING":
    st.header("MARKETING")
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("BANNERS DA LOJA")
        st.subheader("BANNERS / FOTOS / VIDEOS / AUDIOS")
        banner = st.file_uploader("Mídia Banner Novo", type=["jpg", "jpeg", "png", "webp", "mp4"])
        if banner:
            os.makedirs("midia", exist_ok=True)
            st.success(f"Prévia: {banner.name}")
            if banner.type.startswith("image"):
                st.image(banner, width=300)

            if st.button("💾 SALVAR E ATUALIZAR NA LOJA", type="primary"):
                with st.spinner("Enviando para a loja..."):
                    nome_limpo = banner.name.replace(" ", "_")
                    caminho_midia = f"midia/{nome_limpo}"
                    ok1, msg1 = salvar_no_github(caminho_midia, banner.getvalue(), f"Novo banner {nome_limpo}")

                    # atualiza config_loja.json no GitHub também
                    ...
                    cfg_bytes = json.dumps(cfg_atual, indent=4, ensure_ascii=False).encode("utf-8")
                    ok2, msg2 = salvar_no_github("config_loja.json", cfg_bytes, f"Atualiza banner para {nome_limpo}")

                    if ok1 and ok2:
                        st.balloons()
                        st.success("✅ BANNER FOI PARA A LOJA! Aguarde 2 minutos e atualize o site!")
                    else:
    ...
    with col2:
        ...
        if st.button("Salvar Promoção"):
            ...
            # salva local e no github
            salvar_json("config_loja.json", cfg_promo)
            cfg_bytes = json.dumps(cfg_promo, indent=4, ensure_ascii=False).encode("utf-8")
            salvar_no_github("config_loja.json", cfg_bytes, "Atualiza promoção")
...
# ========= FINANCEIRO (MANTIDO IGUAL) =========
    ...
    with st.form("fin"):
        ...
        if st.form_submit_button("Salvar Financeiro", type="primary"):
            ...
            cfg_bytes = json.dumps(cfg, indent=4, ensure_ascii=False).encode("utf-8")
            salvar_no_github("config_loja.json", cfg_bytes, "Atualiza financeiro")
...
# ========= PRODUTOS =========
    ...
    with abas[0]:
        ...
        foto_prod = st.file_uploader("Foto do Produto", type=["jpg","jpeg","png"], key="foto_cad")
        link_img = st.text_input("Ou Link da Imagem (opcional)")
        if st.button("Cadastrar Produto"):
            if foto_prod:
                caminho = os.path.join("midia", foto_prod.name)
                os.makedirs("midia", exist_ok=True)
            ...
            prod_bytes = json.dumps(produtos, indent=4, ensure_ascii=False).encode("utf-8")
            salvar_no_github("produtos.json", prod_bytes, f"Cadastra {nome}")
...
# ========= CONFIGURAÇÃO =========
    ...
    if st.button("Salvar nova senha", type="primary"):
        if atual!= SENHA_ADMIN:
            ...
            st.error("Senha muito curta, mínimo 4 letras!")
        else:
